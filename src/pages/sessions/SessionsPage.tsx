...
const STATUS_NEXT: Record<string, { label: string; next: string; icon: any; cls: string }[]> = {
    open: [{ label: 'Hoàn thành', next: 'completed', icon: CheckCircle, cls: 'text-green-600' },
    { label: 'Hủy', next: 'cancelled', icon: XCircle, cls: 'text-red-500' }],
    full: [{ label: 'Mở lại', next: 'open', icon: PlayCircle, cls: 'text-blue-600' },
    { label: 'Hoàn thành', next: 'completed', icon: CheckCircle, cls: 'text-green-600' }],
    cancelled: [],
    completed: [],
};
                            ...
                            <div key={s.id} className="card flex flex-col gap-3">
                                ...
                                <div className="flex items-center justify-between border-t border-gray-100 pt-2">
                                    <span className="font-semibold text-blue-600 text-sm">
                                        {s.price_male && s.price_female ? (
                                            <>
                                                <span className="text-blue-600">👨 {s.price_male.toLocaleString('vi-VN')}đ</span>
                                                {' · '}
                                                <span className="text-pink-500">👩 {s.price_female.toLocaleString('vi-VN')}đ</span>
                                            </>
                                        ) : (
                                            <>{s.price_per_slot.toLocaleString('vi-VN')}đ / slot</>
                                        )}
                                    </span>
                                ...
                                <div className="flex items-center gap-1 pt-1">
                                    ...
                                    {nextActions.map(({ label, next, icon: Icon, cls }) => (
                                        <button
                                            key={next}
                                            onClick={() => handleStatusChange(s.id, next)}
                                            disabled={busy}
                                            title={label}
                                            className={`p-1.5 rounded-lg hover:bg-gray-100 transition-colors ${cls} disabled:opacity-40`}
                                        >
